...
--
-- This never touches the private production project or the showcase
-- project — it's for exercising every screen (dashboard, accounts,
-- transactions, income, forecast) against a realistic amount of data
-- without hand-entering it through the UI.
--
-- Idempotent: deletes any prior seed user by email first, so re-running
-- this without a full `db reset` just replaces the demo data. Everything
-- else (accounts, transactions, income sources, categories, rules) cascades
-- from that delete via the schema's ON DELETE CASCADE on user_id.
--
...
begin
  ...
  -- function reads auth.uid(), which isn't set in this seed context, so the
