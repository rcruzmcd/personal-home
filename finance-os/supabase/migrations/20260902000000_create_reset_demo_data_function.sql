-- Consolidates the demo-user + sample-data creation logic previously
-- living only in supabase/seed.sql into a reusable function, so local dev
-- seeding (`supabase db reset` -> seed.sql -> `select reset_demo_data();`)
-- and the nightly production reset (pg_cron, see
-- 20260902000001_schedule_demo_reset.sql) share one definition instead of
-- two copies drifting apart.
--
-- security definer, same convention as seed_default_categories()/
-- handle_new_user() in 20260831000003_seed_default_categories.sql:
-- deleting/inserting into auth.users/auth.identities and writing app
-- tables directly (bypassing RLS) requires the privilege of the owning
-- role, not whatever role calls this.
--
...
begin
  ...
  insert into accounts (
    ...
    credit_limit, interest_rate, minimum_payment, due_date, statement_date
  ) values (
    v_user_id, 'Chase Sapphire Reserve', 'Chase', 'credit_card', 7250.00, true, current_date - interval '3 years',
    12000.00, 24.99, 215.00, current_date + interval '13 days', current_date - interval '2 days'
  )
  ...
  insert into accounts (
    ...
    26000.00, 22.99, 610.00, current_date + interval '20 days', current_date - interval '9 days'
  )
  ...
  insert into accounts (
    ...
    6.9, 310.00, 24000.00, 9800.00, 72
  )
  ...
  insert into accounts (
    ...
    6.25, 1850.00, 310000.00, 285000.00, 360
  )
  ...
  values
    ...
    (v_user_id, 'Health Insurance', 'Kaiser Permanente', 195.00, 'monthly', date_trunc('month', current_date) + interval '1 month', v_cat_insurance, v_acct_checking, date_trunc('month', current_date), 4),
    (v_user_id, 'Netflix', 'Netflix', 20.00, 'monthly', date_trunc('month', current_date) + interval '1 month', v_cat_entertainment, v_acct_checking, date_trunc('month', current_date), 4),
    (v_user_id, 'Spotify', 'Spotify', 12.00, 'monthly', date_trunc('month', current_date) + interval '1 month', v_cat_entertainment, v_acct_checking, date_trunc('month', current_date), 4),
    (v_user_id, 'Gym Membership', '24 Hour Fitness', 50.00, 'monthly', date_trunc('month', current_date) + interval '1 month', v_cat_fitness, v_acct_checking, date_trunc('month', current_date), 4);
