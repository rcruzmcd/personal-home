-- Seeds the budgets table in the demo data (added by
-- 20260904000000_create_budgets.sql).
--
-- A full copy of reset_demo_data() rather than an edit to
-- 20260903000001_reset_demo_data_statements.sql, for the same reason that one
...
--
-- The budgets table needs no entry in a delete list: this function opens by
-- deleting the demo auth.users row, and budgets.user_id cascades from it.
--
-- Limits are chosen against the transaction volumes generated further down so
-- the demo shows every budget state on ANY day of the month. That matters
-- because pg_cron re-runs this daily (20260902000001_schedule_demo_reset.sql)
-- and month-to-date spend is tiny on the 2nd: the deterministic states come
-- from the fixed monthly bills, which all post on the 1st --
--   Insurance  $315.00 billed vs a $300 limit -> over from day 1
--   Housing  $1,992.50 billed vs a $2,000 limit -> 99%, near, from day 1
--   Phone/Internet $135.00 vs $140 -> 96%, near, from day 1
-- while the variable categories (Food, Dining Out, Shopping) fill up as the
-- month progresses and cross their limits late in it.
...
begin
  ...
  -- (20260904000000_create_budgets.sql). Sized against the generated spend
  -- below so the page, the dashboard widget and the inbox alerts all have
  -- something real to render -- see this migration's header for which states
  -- are deterministic on any given day.
  --
  -- Childcare is deliberately left unbudgeted so the page's "no limit yet"
  -- treatment has a category to show.
  insert into budgets (user_id, category_id, amount) values
    (v_user_id, v_cat_housing, 2000.00),
    (v_user_id, v_cat_food, 700.00),
    (v_user_id, v_cat_transportation, 200.00),
    (v_user_id, v_cat_insurance, 300.00),
    (v_user_id, v_cat_healthcare, 75.00),
    (v_user_id, v_cat_phone, 140.00),
    (v_user_id, v_cat_dining, 250.00),
    (v_user_id, v_cat_entertainment, 120.00),
    (v_user_id, v_cat_shopping, 200.00),
    (v_user_id, v_cat_subscriptions, 60.00),
    (v_user_id, v_cat_personal_care, 100.00),
    (v_user_id, v_cat_fitness, 75.00);
