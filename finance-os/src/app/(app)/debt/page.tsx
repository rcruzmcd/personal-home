  ...
  return (
    <main className="flex-1 flex flex-col gap-6 px-10 py-16">
      <PageHeader
        ...
        stats={
          <>
            <Stat label="Total debt" value={formatCurrency(summary.totalDebt)} />
            <Stat
              label="Minimum payments"
              value={`${formatCurrency(summary.totalMinimumPayments)}/mo`}
            />
          </>
        }
        actions={
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card variant="featured">
          ...
          {/* The total and the minimum-payment figure now live in the page
              header — repeating them here would say each number twice. */}
          <CardContent className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between border-t border-border pt-4 mt-1">
              <p className="text-body text-muted">Estimated interest</p>
