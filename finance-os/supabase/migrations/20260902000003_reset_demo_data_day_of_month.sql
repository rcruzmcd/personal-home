-- Replaces reset_demo_data() so the demo accounts carry due_day/statement_day
-- instead of the due_date/statement_date columns dropped by
-- 20260902000002_accounts_day_of_month.sql.
--
-- This is a full copy of the function rather than an edit to
-- 20260902000000_create_reset_demo_data_function.sql, because that migration
-- has already been applied, declares the function with a bare `create function`
-- and self-executes it at the end. Editing it in place would make a fresh
-- `supabase db reset` insert into columns that no longer exist at that point in
-- the replay. plpgsql has no partial replacement, so the two files are meant to
-- drift and the later one wins.
--
-- Substantive changes beyond the column rename:
--   * The credit cards get fixed billing days (Sapphire closes the 17th / due
--     the 14th; Amex closes the 28th / due the 3rd) rather than dates offset
--     from current_date, which drifted stale between resets.
--   * Toyota Auto Loan is due on the 31st and Home Mortgage on the 1st, so the
--     calendar shows non-card bills — and the 31st exercises the February clamp.
--   * The recurring expenses' next_date values are staggered across the month.
--     All five previously shared the 1st of next month, which stacked every
--     chip on one calendar cell and left the current month empty.
