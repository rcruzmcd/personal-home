...
import { getWorkProjects, getProjectWithContent } from "@/lib/content/projects"
...
export function generateStaticParams() {
  return LOCALES.flatMap((locale) =>
    getWorkProjects(locale).map((project) => ({ locale, slug: project.slug }))
  )
}
...
export async function generateMetadata({
  ...
  const result = getProjectWithContent("work", slug, locale)
  if (!result) return {}
...
export default async function ProjectDetailPage({
  ...
  const result = getProjectWithContent("work", slug, assertLocale(locale))
  if (!result) notFound()
